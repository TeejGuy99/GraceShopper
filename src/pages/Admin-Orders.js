              ...
              <div key={order.id}>
                <div className="orders" style={{display: 'flex', width: '80vw', justifyContent: 'space-between', margin: '20px'}}>
                  ...
                  <div className='orderProductMap' style={{width: '30vw', textAlign: 'center'}}>{order.products.map((product => {
                    return (
                      <p style={{display: 'inline'}} key={product.id}>{product.id < order.products.length+1 ? `${product.name} (${product.productQty}),`: `${product.name} (${product.productQty})`}</p>
                    )
                  }))}</div>
                  <p style={{width: '10vw', textAlign: 'center'}}>Placeholder</p>
                  <p style={{width: '15vw', textAlign: 'center'}}>Placeholder</p>
                  <p style={{width: '10vw', textAlign: 'center'}}>Placeholder</p>
                </div>
